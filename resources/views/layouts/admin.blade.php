...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .nav-link:hover { background: rgba(220,38,38,0.15); color: #e2e8f0; }
        .nav-link.active { background: rgba(220,38,38,0.25); color: #60a5fa; font-weight: 600; border-left: 3px solid #ef4444; }

        /* ── Header ── */
        header { background: rgba(11,17,32,0.95); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(220,38,38,0.15); }
        ...
        .stat-card:hover { transform: translateY(-3px); box-shadow: 0 8px 32px rgba(220,38,38,0.15); border-color: rgba(220,38,38,0.3); }
        .card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 16px; }

        /* ── Buttons ── */
        .btn-primary { background: linear-gradient(135deg, #dc2626, #b91c1c); color: white; border-radius: 10px; padding: .625rem 1.5rem; font-weight: 600; transition: all .2s; box-shadow: 0 4px 16px rgba(220,38,38,0.35), inset 0 1px 0 rgba(255,255,255,0.1); display:inline-flex; align-items:center; gap:.4rem; }
        .btn-primary:hover { transform: translateY(-1px); box-shadow: 0 6px 24px rgba(220,38,38,0.5); }
        ...
        .btn-secondary:hover { border-color: rgba(220,38,38,0.4); color: #e2e8f0; }
        ...
        .input-field:focus { outline: none; border-color: #ef4444; box-shadow: 0 0 0 3px rgba(239,68,68,.15); }
        ...
        .quota-fill { height:100%; border-radius:99px; background:linear-gradient(90deg,#ef4444,#dc2626); transition:width .6s ease; }
        ...
        /* ── Chatbot ── */
        #chatbot-btn { position:fixed; bottom:28px; right:28px; z-index:9999; width:58px; height:58px; border-radius:50%; background:linear-gradient(135deg,#dc2626,#b91c1c); box-shadow:0 8px 32px rgba(220,38,38,0.5); border:2px solid transparent; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:1.6rem; transition:all .3s cubic-bezier(0.34,1.56,0.64,1); }
        #chatbot-btn:hover { transform:scale(1.12); box-shadow:0 12px 40px rgba(220,38,38,0.7); }
        #chatbot-btn.active {
            background: #0f172a !important;
            border-color: #dc2626 !important;
            box-shadow: 0 0 24px rgba(220,38,38,0.6) !important;
            transform: scale(0.92) !important;
        }
        #chatbot-panel { position:fixed; bottom:28px; right:100px; z-index:9998; width:360px; max-height:520px; background:#0f172a; border:1px solid rgba(220,38,38,0.3); border-radius:20px; box-shadow:0 24px 80px rgba(0,0,0,.6); display:flex; flex-direction:column; overflow:hidden; transition:all .3s cubic-bezier(0.34,1.56,0.64,1); transform-origin:bottom right; }
        #chatbot-panel.hidden { opacity:0; transform:scale(0.85); pointer-events:none; }
        .chat-msg-bot { background:rgba(220,38,38,0.12); border:1px solid rgba(220,38,38,0.2); border-radius:14px 14px 14px 4px; padding:.65rem 1rem; color:#93c5fd; font-size:.875rem; max-width:85%; align-self:flex-start; }
        .chat-msg-user { background:rgba(220,38,38,0.9); border-radius:14px 14px 4px 14px; padding:.65rem 1rem; color:white; font-size:.875rem; max-width:85%; align-self:flex-end; }
        ...
        .tour-card { background:#0f172a; border:1px solid rgba(220,38,38,0.4); border-radius:24px; padding:2.5rem; max-width:480px; width:90%; box-shadow:0 32px 80px rgba(0,0,0,.8); }
        .tour-step-dot { width:8px; height:8px; border-radius:50%; background:rgba(255,255,255,0.15); transition:all .3s; }
        .tour-step-dot.active { background:#ef4444; width:20px; border-radius:99px; }

        /* ── Scrollbar ── */
        ::-webkit-scrollbar { width:5px; } ::-webkit-scrollbar-track { background:#0b1120; } ::-webkit-scrollbar-thumb { background:#b91c1c; border-radius:99px; }
        ...
        body.light-mode .nav-link.active { background: #fee2e2; color: #b91c1c; }
        ...
        body.light-mode .bg-red-900\/20 { background: rgba(220,38,38,0.05); }
        body.light-mode .border-red-900\/30 { border-color: #e2e8f0; }
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="sidebar w-60 flex-shrink-0 flex flex-col" id="sidebar">
        {{-- Logo --}}
        <a href="{{ route('dashboard') }}" class="px-5 py-5 flex items-center gap-3 border-b border-red-900/30 hover:bg-white/5 transition-colors group">
            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-red-600 to-red-900 flex items-center justify-center text-lg shadow-lg group-hover:scale-110 transition-transform logo-icon">🇮🇳</div>
            <div>
                <div class="font-bold text-white leading-tight">JanBhasha</div>
                <div class="text-xs text-red-400" style="font-family:'Noto Sans Devanagari',sans-serif;">जनभाषा</div>
            </div>
        </a>
        ...
        <div class="mx-3 mt-4 mb-1 bg-red-900/20 border border-red-800/30 rounded-xl px-4 py-2.5">
            <div class="text-xs text-red-500 uppercase tracking-wide font-medium mb-0.5">Organisation</div>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto" id="main-nav">
            ...
            <div class="pt-4 pb-1 px-4 text-xs text-red-500 uppercase tracking-wide font-semibold">Admin</div>
        ...
        {{-- User footer --}}
        @auth
        <div class="border-t border-red-900/30 px-3 py-3">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="px-8 py-4 flex items-center justify-between flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <button id="sidebar-toggle" onclick="toggleSidebar()" class="w-9 h-9 rounded-xl bg-red-900/20 border border-red-800/30 items-center justify-center hover:border-red-500 transition-all" aria-label="Menu">
            ...
            <div class="flex items-center gap-3">
                <button onclick="toggleTheme()" class="w-10 h-10 rounded-xl bg-red-900/20 border border-red-800/30 flex items-center justify-center hover:border-red-500 transition-all group" title="Switch Mode">
...
<div id="chatbot-panel" class="hidden">
    <div class="px-5 py-4 border-b border-red-900/40 flex items-center justify-between bg-red-950/40">
        <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center text-sm">🤖</div>
            <div>
    ...
    <div id="chat-messages" class="flex-1 overflow-y-auto p-4 flex flex-col gap-3" style="min-height:280px;max-height:340px;">
        ...
        <div class="flex flex-wrap gap-2 mt-1">
            <button onclick="askBot('how to translate')" class="text-xs bg-red-900/40 border border-red-800/40 text-red-300 rounded-full px-3 py-1 hover:bg-red-800/40 transition-colors">How to translate?</button>
            <button onclick="askBot('what is glossary')" class="text-xs bg-red-900/40 border border-red-800/40 text-red-300 rounded-full px-3 py-1 hover:bg-red-800/40 transition-colors">What is Glossary?</button>
            <button onclick="askBot('api key')" class="text-xs bg-red-900/40 border border-red-800/40 text-red-300 rounded-full px-3 py-1 hover:bg-red-800/40 transition-colors">API Key?</button>
            <button onclick="askBot('quota')" class="text-xs bg-red-900/40 border border-red-800/40 text-red-300 rounded-full px-3 py-1 hover:bg-red-800/40 transition-colors">About Quota?</button>
        </div>
    </div>
    <div class="p-3 border-t border-red-900/40">
        <form onsubmit="sendChat(event)" class="flex gap-2">
            <input id="chat-input" type="text" placeholder="Ask a question..." class="flex-1 bg-red-900/20 border border-red-800/30 rounded-xl px-4 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-red-500">
            <button type="submit" class="w-9 h-9 rounded-xl bg-red-600 hover:bg-red-500 flex items-center justify-center text-white text-sm transition-colors">→</button>
        </form>
    </div>
</div>
